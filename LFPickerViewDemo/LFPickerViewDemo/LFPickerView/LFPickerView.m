...
@interface LFPickerView ()<UIPickerViewDelegate,UIPickerViewDataSource>
...
//选中县级对应的下标
@property (nonatomic,assign) NSInteger selecRowWithTown;
...
-(CityDataModel *)cityModel{
    ...
    if (_cityModel==nil) {
        MySingleton *mySing=[MySingleton shareMySingletion];
        if (mySing.cityModel) {
            ...
            NSDictionary *dicValue=[mySing getObjectFormJsonString:stringValue];  // 将本地JSON数据转为对象
            _cityModel=[CityDataModel mj_objectWithKeyValues:dicValue];
...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        //选择titi
        self.selectLbl = [UILabel new];
        [self.bgV addSubview:self.selectLbl];
        ...
        self.selectLbl.font = [UIFont systemFontOfSize:kfont];
        self.selectLbl.textAlignment = NSTextAlignmentCenter;
...
- (void)setCustomArr:(NSArray *)customArr{
    ...
    [self.bgV addSubview:self.pickerView];
...
- (void)setArrayType:(ARRAYTYPE)arrayType
{
    ...
    switch (arrayType) {
        ...
        case GenderArray:
        {
            self.selectLbl.text = @"选择性别";
            [self.array addObject:@[@"男",@"女"]];
        }
            break;
        case HeightArray:
        {
            self.selectLbl.text = @"选择身高";
        ...
        case DeteArray:
        {
            self.selectLbl.text = @"选择出生年月";
        }
            break;
            
        case AreaArray:
        {
            self.selectLbl.text = @"选择省市区";
            [self getAreaData];
        }
            break ;
        default:
...
-(NSString *)finaSureCity{
    ...
    if (self.selectRowWithProvince<self.allProvince.count) {
        ...
        if (self.selectRowWithCity<provinceValue.city.count) {
            ...
            if (self.selecRowWithTown<cityValue.district.count) {
                District *dictrict=cityValue.district[self.selecRowWithTown];
